...
contract GNSTradingStorageV5 is StorageInterfaceV5, IStateCopyUtils, Initializable {
    ...
    mapping(uint => PendingMarketOrder) public _reqID_pendingMarketOrder;
    mapping(uint => PendingNftOrder) public _reqID_pendingNftOrder;
    ...
    // Modifiers
    modifier onlyGov(){require(msg.sender == gov);
        _;}
    modifier onlyTrading(){require(isTradingContract[msg.sender] && token.hasRole(MINTER_ROLE, msg.sender));
        _;}
        ...
        for (uint i = 0; i < orderIds.length; i++) {
            if (orderIds[i] == _id) {
                if (_open) {
                    ...
                    pendingMarketCloseCount[_order.trade.trader][_order.trade.pairIndex]--;
                    _openTradesInfo[_order.trade.trader][_order.trade.pairIndex][_order.trade.index].beingMarketClosed = false;
                }
    ...
    function updateOpenLimitOrder(OpenLimitOrder calldata _o) external onlyTrading {
        ...
        OpenLimitOrder storage o = _openLimitOrders[openLimitOrderIds[_o.trader][_o.pairIndex][_o.index]];
    ...
    function unregisterOpenLimitOrder(address _trader, uint _pairIndex, uint _index) external onlyTrading {
        ...
        _openLimitOrders[id] = _openLimitOrders[_openLimitOrders.length - 1];
        openLimitOrderIds[_openLimitOrders[id].trader][_openLimitOrders[id].pairIndex][_openLimitOrders[id].index] = id;

        // Remove
        delete openLimitOrderIds[_trader][_pairIndex][_index];
        _openLimitOrders.pop();
    ...
    function storePendingNftOrder(PendingNftOrder memory _nftOrder, uint _orderId) external onlyTrading {
        _reqID_pendingNftOrder[_orderId] = _nftOrder;
    }

    function unregisterPendingNftOrder(uint _order) external onlyTrading {
        delete _reqID_pendingNftOrder[_order];
    }
    ...
    function handleTokens(address _a, uint _amount, bool _mint) external onlyTrading {
        if (_mint) {token.mint(_a, _amount);
            tokensMinted += _amount;}
        else {token.burn(_a, _amount);
    ...
    function hasOpenLimitOrder(address trader, uint pairIndex, uint index) public view returns (bool){
        if (_openLimitOrders.length == 0) {return false;}
        OpenLimitOrder storage o = _openLimitOrders[openLimitOrderIds[trader][pairIndex][index]];
        return o.trader == trader && o.pairIndex == pairIndex && o.index == index;
    }

    // Additional getters
    function openTrades(address trader, uint pairIndex, uint index) external view override returns (Trade memory) {
        return _openTrades[trader][pairIndex][index];
    }

    function openTradesInfo(address trader, uint pairIndex, uint index) external view override returns (TradeInfo memory) {
        return _openTradesInfo[trader][pairIndex][index];
    }

    function openLimitOrders(uint orderId) external view override returns (OpenLimitOrder memory) {
        return _openLimitOrders[orderId];
    }

    function reqID_pendingMarketOrder(uint id) external view override returns (PendingMarketOrder memory) {
        return _reqID_pendingMarketOrder[id];
    }

    function reqID_pendingNftOrder(uint id) external view override returns (PendingNftOrder memory) {
        return _reqID_pendingNftOrder[id];
    }
    ...
    function getOpenLimitOrder(
        ...
        return _openLimitOrders[openLimitOrderIds[_trader][_pairIndex][_index]];
    }

    function getOpenLimitOrders() external view returns (OpenLimitOrder[] memory){
        return _openLimitOrders;
    }

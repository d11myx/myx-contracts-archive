...
contract PairVault is IPairVault, Handleable {
    ...
    function setSlipReceiver(address _slipReceiver) external onlyOwner {
    ...
    function createPair(address token0, address token1) external returns (address) {
        ...
        IPairToken(pairToken).initialize(token0, token1);
        return pairToken;
    }
    ...
    function removeLiquidity(uint256 _pairIndex, uint256 _amount) external returns (uint256, uint256) {
        return _removeLiquidity(msg.sender, msg.sender, _pairIndex, _amount);
    }

    function removeLiquidity(address _account, address _receiver, uint256 _pairIndex, uint256 _amount) external onlyHandler returns (uint256, uint256) {
    ...
    function _addLiquidity(address _funder, address _account, uint256 _pairIndex, uint256 _indexAmount, uint256 _stableAmount) private returns (uint256) {
        ...
        // usdt value of reserve
        {
            ...
            uint256 lpFairPrice = lpFairDelta > 0 ? Math.mulDiv(lpFairDelta, PRICE_PRECISION, IERC20(pair.pairToken).totalSupply()) : lpFairDelta;
            // mint lp
        ...
        vault.indexReservedAmount = vault.indexReservedAmount + afterFeeIndexAmount;
        vault.stableTotalAmount = vault.stableTotalAmount + afterFeeStableAmount;
        vault.stableReservedAmount = vault.stableReservedAmount + afterFeeStableAmount;
    ...
    function _removeLiquidity(address _account, address _receiver, uint256 _pairIndex, uint256 _amount) private returns (uint256, uint256) {
        ...
        vault.indexReservedAmount = vault.indexReservedAmount - receiveIndexTokenAmount;
        vault.stableTotalAmount = vault.stableTotalAmount - receiveStableTokenAmount;
        vault.stableReservedAmount = vault.stableReservedAmount - receiveStableTokenAmount;

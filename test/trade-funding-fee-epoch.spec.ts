...
import { Duration, increase, TradeType, getAveragePrice } from '../helpers';
...
describe('Trade: funding fee epoch', () => {
    ...
    describe('calculate whether different prices will achieve balance', async () => {
        ...
        it('epoch 2, 26000 price open position', async () => {
            ...
            let shortPosition = await positionManager.getPosition(short.address, pairIndex, false);
            ...
            expect(globalFundingFeeTracker).to.be.gt(0);

            longFirstPosition = await positionManager.getPosition(longFirst.address, pairIndex, true);
            longSecondPosition = await positionManager.getPosition(longSecond.address, pairIndex, true);
            shortPosition = await positionManager.getPosition(short.address, pairIndex, false);

            expect(longFirstPosition.fundingFeeTracker)
                .to.be.eq(0)
                .and.eq(longSecondPosition.fundingFeeTracker)
                .and.eq(shortPosition.fundingFeeTracker);
        });
        ...
        it('epoch 3, 26500 price increase position', async () => {
            ...
            await mintAndApprove(testEnv, usdt, collateral, longFirst, router.address);
            await increasePosition(testEnv, longFirst, pairIndex, collateral, openPrice, size, TradeType.MARKET, true);
            let position = await positionManager.getPosition(longFirst.address, pairIndex, true);

            expect(position.positionAmount).to.be.eq('25000000000000000000');
            expect(position.averagePrice).to.be.lt(openPrice);

            let globalFundingFeeTracker = await positionManager.globalFundingFeeTracker(pairIndex);
            expect(globalFundingFeeTracker).to.be.eq(position.fundingFeeTracker);
            ...
            globalFundingFeeTracker = await positionManager.globalFundingFeeTracker(pairIndex);
            expect(globalFundingFeeTracker).to.be.gt(position.fundingFeeTracker);

            const fundingFee = await positionManager.getFundingFee(longFirst.address, pairIndex, true);
            expect(fundingFee).to.be.lt(0);
        });
        ...
        it('epoch 4, 25000 price increase position', async () => {
            ...
            const price = ethers.utils.parseUnits('25000', 30);
            ...
            expect(longFirstPosition.averagePrice).to.be.gt(price);
            expect(longSecondPosition.averagePrice).to.be.gt(price);
            expect(shortPosition.averagePrice).to.be.gt(price);
        });
        ...
        it('epoch 5, 24000 price increase position', async () => {
            ...
            const price = ethers.utils.parseUnits('24000', 30);

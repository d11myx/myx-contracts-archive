...
import { FullMathTest } from "../typechain-types/FullMathTest";
...
describe("FullMath", () => {
    ...
    describe("#mulDiv", () => {
        ...
        it("reverts on overflow with all max inputs", async () => {
            await expect(
                fullMath.mulDiv(MaxUint256, MaxUint256, MaxUint256.sub(1))
            ).to.be.reverted;
        });

        it("all max inputs", async () => {
            expect(
                await fullMath.mulDiv(MaxUint256, MaxUint256, MaxUint256)
            ).to.eq(MaxUint256);
        });
